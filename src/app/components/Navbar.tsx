    ...
    <nav className={`${styles.paddingX} w-full flex items-center py-5 fixed top-0 z-20 bg-background`}>
      <div className='w-full flex justify-between items-center max-w-7xl mx-auto'>
        <Link href="/" className='flex items-center gap-2' onClick={() => {setActive;
          ...
          <p className='text-white text-[18px] font-bold cursor-pointer flex'> Render Gap&nbsp; <span className='sm:block hidden'> | 3D Specialist</span></p>
        </Link>
        ...
        <div className='sm:hidden flex flex-1 justify-end items-center'>
          <img src={toggle ? close.src : menu.src} 
            alt="menu"
           className='w-[28px] h-[28px] object-contain cursor-pointer'
           onClick={()=>setToggle(!toggle)}/>
           <div className={`${!toggle ? 'hidden' : 'flex'} p-6 bg-gradient-to-b from-black to-gray-900 absolute top-20 right-0 mx-4 my-2 min-w-[140px] z-10 rounded-xl`}>
